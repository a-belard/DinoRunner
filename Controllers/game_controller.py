...
class GameController(BaseController):
    ...
    def handleKeyPress(self, key):
        ...
        # Game flow controls (pause, restart)
    ...
    def update(self):
        # Always update the game flow controller for pause/restart controls
        self.gameFlowController.update()
        
        # Always update dino controller for death animation
        if hasattr(self, 'dinoController') and hasattr(self.dinoController, 'update'):
            self.dinoController.update()
        
        # Only update game elements when not on start screen, paused, or game over
        if not (self.app.isStartScreen or self.app.isPaused or self.app.isGameOver):
